...
3. A PNG chart saved to `pl_pricing_wow.png` (always generated by default)

```bash
python weekly_report.py                        # default: origination week, all segments
python weekly_report.py --lp-or-mpl MPL        # MPL only
python weekly_report.py --output my_file.png   # override chart filename
```
