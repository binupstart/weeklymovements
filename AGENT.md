...
2. **Origination fee (`combined_fee_rate` / ofee)** — The fee charged to the borrower for the loan. This can be charged explicitly (deducted upfront) or implicitly. **Important:** `combined_fee_rate` is the raw (non-annualized) ofee. The APR component is the annualized ofee, which requires dividing by loan term and adjusting for prepayment assumptions. This means `combined_fee_rate` does not move 1:1 with the fee component of APR — a shift in loan term mix (e.g. more 3-year vs. 5-year loans) can change the annualized fee and APR without any change to `combined_fee_rate`. When fee and APR diverge directionally, check whether loan term mix or annualization assumptions shifted, not just the nominal fee rate.
